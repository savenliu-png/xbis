...
| **TrendChart** | 趋势图表：支持日/周/月切换 | 高 |
| **AlertItem** | 告警项：级别 + 消息 + 时间 | 低 |
...
- TrendChart 使用 Canvas/SVG，限制数据点数量（最多 30 个）
...
| 风险 | 等级 | 说明 | 应对 |
|------|------|------|------|
| 图表性能 | 中 | 数据量大时图表卡顿 | 限制数据点，使用 Canvas |
| 数据实时性 | 低 | 5 分钟刷新可能延迟 | 接受延迟，或缩短到 1 分钟 |
