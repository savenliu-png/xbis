...
| 组件 | 用途 | 说明 |
|------|------|------|
| SchemaEditor | Schema 可视化编辑 | 渐进式实现：第一阶段支持基础类型（string/number/boolean），后续支持对象/数组 |
| FormBuilder | UI 表单构建器 | 渐进式实现：第一阶段支持基础字段配置 |

### 2.3 blocks/ 层组件（需新建）
| 组件 | 用途 | 说明 |
|------|------|------|
| BasicInfoTab | 基础信息 Tab | 名称/描述/分类/标签/图标/风险等级 |
| SchemaTab | 输入输出 Schema Tab | 包含 SchemaEditor |
| UiConfigTab | UI 配置 Tab | 包含 FormBuilder |
| ExecutionTab | 执行配置 Tab | 执行器/超时/重试 |
| BillingTab | 计费配置 Tab | 价格/配额/阶梯定价 |
| PublishTab | 发布管理 Tab | 版本/状态/变更日志/版本历史 |

### 2.4 组件复用关系
```
SchemaEditor (business)
└── 基于开源库 react-jsonschema-form 或自研简化版
    ├── 第一阶段支持类型：string/number/boolean
    ├── 后续支持类型：array/object
...
| 版本历史 | GET | `/admin-api/v1/abilities/:id/versions` | 进入 PublishTab | — | Toast 错误提示 |
...
| 优化点 | 策略 | 说明 |
|--------|------|------|
| 首屏加载 | 并行加载能力数据 + 执行器列表 | 减少等待时间 |
| Tab 切换 | 懒加载 SchemaEditor/FormBuilder | 首屏只加载当前 Tab |
| 代码分割 | SchemaEditor/FormBuilder 动态导入 | 减少首屏包体积 |

### 8.2 运行时优化
| 优化点 | 策略 | 说明 |
|--------|------|------|
| 自动保存 | Debounce 3s | 避免频繁请求 |
| 表单校验 | 当前 Tab 实时校验，其他 Tab 保存时校验 | 减少计算 |
| 内存管理 | 离开页面清理 SchemaEditor 实例 | 防止内存泄漏 |

### 8.3 渲染优化
| 优化点 | 策略 | 说明 |
|--------|------|------|
| Tab 内容 | 非活跃 Tab 保持挂载但暂停响应式更新 | 条件渲染 |
| SchemaEditor | 大数据量 Schema 使用虚拟滚动 | 性能优化 |
| 表单控件 | 使用 React.memo 减少重渲染 | 渲染优化 |
...
| 风险 | 说明 | 应对方案 |
|------|------|----------|
| SchemaEditor 复杂度高 | 需要支持 JSON Schema 完整语法，开发工作量大 | 渐进式实现：第一阶段基础类型（string/number/boolean），后续扩展对象/数组 |
...
| 自动保存冲突 | 用户操作和自动保存同时触发 | Debounce + 队列机制，确保顺序执行；手动保存等待自动保存完成 |
...
- [ ] 实现 BillingTab（计费配置）
- [ ] 实现 PublishTab（发布管理 + 版本历史）

### Step 3: 复杂编辑器（1.5 天）
- [ ] 集成/开发 SchemaEditor（第一阶段：基础类型 string/number/boolean）
- [ ] 集成/开发 FormBuilder（第一阶段：基础字段配置）
- [ ] 实现编辑器与表单数据的双向绑定
...
packages/shared/src/api/services.ts         # 添加 adminApi.abilities.get, update, publish, versions
