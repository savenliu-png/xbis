...
| 组件 | 用途 | 说明 |
|------|------|------|
| SlaDashboard | SLA 统计面板 | 4 个统计卡片 |
| OperationTable | 运营任务表格 | 基于 Table，含批量操作 |
| ReviewPanel | 审核面板 | Modal 内审核表单 |
| RefundPanel | 退款面板 | Modal 内退款表单 |
| BatchActionBar | 批量操作栏 | 顶部批量操作 |
...
| 操作日志 | POST | `/admin-api/v1/operations/logs` | 审核/退款后 | 静默失败 |
...
| 风险 | 说明 | 应对方案 |
|------|------|----------|
| 误操作审核/退款 | 影响用户资金和任务执行 | 二次确认 Modal；敏感操作记录日志；退款金额校验 |
...
### Step 2: 列表与筛选（1 天）
- [ ] 实现 OperationTable（含批量操作）
- [ ] 实现筛选栏（含时间范围）
- [ ] 实现批量操作栏

### Step 3: 操作面板（1.5 天）
- [ ] 实现 ReviewPanel（审核 Modal + 操作日志）
- [ ] 实现 RefundPanel（退款 Modal + 金额校验）
- [ ] 实现重试逻辑
