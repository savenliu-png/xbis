# T028 管理端执行器管理 — 工程级设计方案（Design Spec）

> 任务卡：T028-admin-executor-management.md  
> 状态：设计确认  
> 输出日期：2026-04-24
...
| 组件 | 用途 | 说明 |
|------|------|------|
| HealthIndicator | 健康状态指示器 | **新建** — 颜色+图标表示 healthy/degraded/unhealthy |
| ResourceChart | 资源使用图表 | **新建** — 迷你 CPU/内存使用率条形图 |

### 2.3 blocks/ 层组件（需新建）

| 组件 | 用途 | 说明 |
|------|------|------|
| ExecutorTable | 执行器表格 | **新建** — 基于 Table，包含展开详情 |
| HealthPanel | 健康详情面板 | **新建** — 展开行/抽屉内展示健康历史 |
| ConfigPanel | 配置编辑面板 | **新建** — 抽屉内编辑执行器配置 |
| BindingPanel | 能力绑定面板 | **新建** — 展示/管理执行器绑定的能力 |
...
| 403 | 权限不足 | 跳转 403 |
| 409 | 执行器正在运行任务 | Modal 提示「有进行中的任务，无法停用」 |
...
| 场景 | 用户行为 | 系统响应 |
|------|----------|----------|
| 停用有任务的执行器 | 点击 Switch | Modal 提示「有 N 个进行中的任务，确认停用？」 |
| 配置值非法 | 输入负数 | 表单校验阻止提交 |
...
| 风险 | 说明 | 应对方案 |
|------|------|----------|
| **误操作停用执行器** | 停用正在运行任务的执行器会导致任务失败 | 二次确认 Modal，显示进行中的任务数；后端校验 |

### 9.2 中风险

| 风险 | 说明 | 应对方案 |
|------|------|----------|
| 健康数据量大 | 历史健康数据可能很大 | 限制展示最近 50 条，支持分页 |
| 配置错误 | 错误的配置可能导致执行器异常 | 前端校验 + 后端校验 |
